...
export function buildArcs(articles) {
  ...
  for (const article of articles) {
    ...
    const titleSet = new Set(article.titleCountries || []);

    // Only draw an arc if at least one country is explicitly in the title.
    // This filters out arcs built from passing mentions in the description.
    const titleMatches = countries.filter((c) => titleSet.has(c.name));
    if (titleMatches.length === 0) continue;

    // Both source and target must be present in the title.
    // This prevents spurious arcs from passing mentions in the description.
    if (titleMatches.length < 2) continue;

    const source = titleMatches[0];
    const target = titleMatches[1];

    if (!source || !target) continue;

    const src = COUNTRY_COORDS[source.name];
    const tgt = COUNTRY_COORDS[target.name];
    if (!src || !tgt) continue;
